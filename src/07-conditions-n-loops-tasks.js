...
function isCreditCardNumber(ccn) {
  ...
  const lastDigit = ccnArr.pop();
  const ccnSum = ccnArr.reverse().map((item, index) => {
    let result = item;
    if (index % 2 === 0) {
      result = item * 2;
      if (result > 9) {
        result = result.toString().split('').map((i) => Number(i)).reduce((prev, sum) => prev + sum, 0);
      }
    }
    return result;
  }).reduce((prev, sum) => prev + sum, 0);
  if ((ccnSum + lastDigit) % 10 === 0) return true;
  return false;
}
...
function getDigitalRoot(num) {
  let result = num;
  while (result > 9) {
    result = result.toString().split('').map((item) => Number(item)).reduce((prev, sum) => prev + sum, 0);
  }
  return result;
}
